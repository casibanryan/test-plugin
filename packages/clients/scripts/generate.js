...
//   node scripts/generate.js --write --client=claude     just one
...
// The CANONICAL server, and the three files a plugin client receives a copy of.
// Everything else in this file writes text it composed; these are copied byte for
// byte, which is what lets `clients:verify` compare a plugin's copy against its source
// and fail on any difference at all.
//
...
// always this checkout's packages/server.
...
// renderAll's entries so the write and drift paths treat them identically — which is
// the point: the server a plugin ships is then under exactly the same drift check as
// the config that points at it.
//
// Two kinds, and the distinction matters when one fails:
//
//   tools.json   COMPILED here from the contract's TOOLS, into packages/server (the
//                canonical copy the repo-relative clients run) and into every plugin.
//   the server   COPIED byte for byte out of packages/server, because a marketplace
//                install receives only the plugin directory and cannot require its way
//                back into this repository.
function bundledArtifacts(channel, only = null) {
  if (channel.transport !== 'stdio') return [];

  const manifest = bundledToolManifest(channel);
  const out = [];

  // The canonical tool surface, next to the server that reads it. Not tied to any one
  // client: Codex and Gemini run this copy directly.
  if (!only) {
    out.push({
      client: { id: 'server' },
  ...
  for (const client of CLIENTS) {
    // Only a plugin client needs its own copy. Codex and Gemini name the canonical
    // path in their config, so duplicating the server for them would be dead weight
    // and a second thing to keep in step.
    if (!client.plugin) continue;
    if (only && only !== client.id) continue;

    const dir = path.join(ROOT, client.id, 'server');
    out.push({ client, file: path.join(dir, 'tools.json'), relative: `${client.id}/server/tools.json`, content: manifest });
    for (const name of COPIED_SERVER_FILES) {
      out.push({
        client,
        file: path.join(dir, name),
        relative: `${client.id}/server/${name}`,
